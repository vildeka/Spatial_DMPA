...
```{r DEGs-SubMuc-vs-epi, include=TRUE, eval=FALSE}
##############################
...
```{r top-10-morf-DEGs, fig.height=8.5, fig.width=8.27}
#############################
...
###################################
DAT <- DATA  %>%
  mutate(., feature = .$sp_annot)
ord <- niceRplots::getcluster(DAT, unique(marker_genes_morf$gene), "feature")

#pdf( paste0("./Figures/05/","Marker_genes_epi_vs_subMuc.pdf"), width=4.27, height=length(ord)/6+2, bg = "transparent")
par(mfrow=c(1,1), mar=c(4,6,1,5))
plot_dots(DAT, names(sort(ord)), clustering = "feature", show_grid = T,main = "top markers",cex.main=1,font.main=1,cex.col = 1,srt = 0,cex.row = 1.1)
...
DEGs_table_morf <- read_csv(paste0(result_dir,"DGEs_clusters_morf_25_25.csv"))
...
```{r 05e_dotplott_top20_DEGs_sm_vs_epi, fig.height=40/6+2, fig.width=4}
#| fig-format: pdf
#######################################
# FILTER BY P-VAL logFC AND pct.diff #
######################################
# Identify the top genes that have a high difference in expression between the clusters
top20_sm_vs_epi <- DEGs_table_morf %>%
  mutate(Direction = ifelse(avg_log2FC > 0, "UP", "DOWN")) %>%
  #filter(Direction == "UP") %>%
  group_by(Direction) %>%
  top_n(-40, p_val_adj) %>%
...
DAT <- DATA  %>%
  #filter(!(grepl(epi_clus, .$Clusters ))) %>%
  mutate(., feature = .$sp_annot)
ord <- niceRplots::getcluster(DAT, unique(top20_sm_vs_epi$gene), "feature")
...
```{r 05f_violin_DEGs_sm_vs_epi, fig.height=ceiling(length(unique(top20_sm_vs_epi$gene))/3)*2, fig.width=7}
...
feature <-  unique(top20_sm_vs_epi$gene)
