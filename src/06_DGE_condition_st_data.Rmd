...
PlosPath_DGEs <- "../data/Bulk_data/DMPA_PlosPath_DEGs.csv"
...
DEGs_all <- read_csv("../results/06_DGE_condition_st_data/DEGs_Zal_Brad_ST.csv")


D <- DEGs_all %>% #filter(study == "Brad") %>% 
  mutate(., layers = factor(l[as.character(.$cluster)], levels = ord1)) %>%
  mutate(Regulation = ifelse(logFC > 0 & FDR < 0.05, "UP",
                             ifelse(logFC < 0 & FDR < 0.05,"DOWN", "NOT SIG."))) %>% nest(data = -Regulation)

D %>% filter(study == "ST") %>% .$symbol %>% unique(.)
...
# Zalenskya sig DEGs
Zal_DEGs <- read_xlsx("../data/Bulk_data/Zalenskaya_DEGs.xlsx", 
...
```{r proteases}
protase_path <- "../data/Protease_antiprotease.xlsx"
...
write.xlsx(protase_df, keepNA=TRUE, na.string="NA", overwrite=TRUE,
           file=paste0("../data/","Protease_antiprotease_modified.xlsx"))
...
#####################################
o <- t$genes %>% set_names(., t$layers) %>% .[-c(2:7)]
o <- t$genes %>% set_names(., t$layers) %>% .[c(2, 5:7)] # SubMucosa
...
cat(paste(g))
genes <-c("PDZK1IP1","CXCL14", "MAOA", "H19")
morf <- "epi"
morf <- "subMuc"
