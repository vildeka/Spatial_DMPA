...
int_genes <- t$sig_DEGs %>%
  getVennOverlap() %>% 
  tibble(name = names(.), Uniqe_int = .) %>%
  mutate(n = map_dbl(.$Uniqe_int, ~length(.x)))

# all four comparisons
d <- plotVenn(t$sig_DEGs, 
              labelRegions=FALSE,
              sNames = names(t$sig_DEGs), systemShow=T) %>%
  showSVG(., 
         setColors = c('#ca0020','#f4a582',"#018571",'#0571b0'),
         labelRegions=FALSE,
         showNumbers=FALSE,
         fontScale = 1,
         systemShow=T
         #outFile=paste0("./Figures/06/", "Venn_E2.svg")
         )

# showSVG(d, labelRegions = T)
knitr::include_graphics(paste0("./Figures/06/", "Venn_E2.svg"))

# Zalenskaya, ST and E2
d <- plotVenn(t$sig_DEGs[2:4], 
              labelRegions=FALSE,
              sNames = names(t$sig_DEGs), systemShow=T) %>%
  showSVG(., 
         setColors = c('#ca0020','#f4a582','#0571b0'),
         labelRegions=FALSE,
         showNumbers=FALSE,
         fontScale = 2,
         systemShow=T
...
```{r}
KEGG_database[["KEGG_RETINOL_METABOLISM"]]

i <- intersect(All_DEGs$symbol, KEGG_database[["KEGG_RETINOL_METABOLISM"]])
print(intersect(All_DEGs$symbol, KEGG_database[["KEGG_RETINOL_METABOLISM"]]))
a <- filter(All_DEGs, symbol %in% i)
```
