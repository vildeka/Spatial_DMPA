...
render_git_with_job(fig_path = "./Figures/S2&3/")
system2(command = "sed", stdout = TRUE,
        args = c("-i", "''","-e", 's/src=\\"\\./src=\\"\\.\\./g',
                 paste0("./md_files/", basename("./S2&3_figures.md"))))

# kniter
knit_html_with_job(out_dir = "../lab_book/figure_S2&3", fig_path = "./Figures/S2&3/")
```
...
```{r S1a_enrichment-graph-epi, fig.width=5.5, fig.height=5}
...
# ggsave(paste0("./Figures/S2/","./enrichment-graph-epi.png"), plot = p, width = 6, height = 5, dpi = 1000 )
p
```


```{r S1b_enrichment-graph-submuc, fig.width=5.5, fig.height=5}
dge <- DEGs_table %>% 
  filter(grepl("[\\d]", .$layers)) %>% 
  filter(p_val<=0.01) %>%
  #filter(pct.1 > .4 & p_val < 0.01)  
...
# dev.new(width=5.5, height=5, noRStudioGD = TRUE)
# ggsave(paste0("./Figures/S2/","./enrichment-graph-submuc.png"), plot = p, width = 6, height = 5, dpi = 1000 )
p
```

```{r S2a_graph-per-layer, fig.width=5, fig.height=15}
...
(p <- wrap_plots(p, ncol = 1) )#
# ggsave(filename = "./Figures/S2/graph_per_layer_sub.pdf", plot = p, width =1 , height =3.9, dpi = 1000, bg = "white" )
```

```{r S2a_piechart-GOslim-graph-clus,  fig.width =10 , fig.height =10}
...
# ggsave(filename = "./Figures/S2/piechart-GOslim-graph-clus_epi.pdf", plot = pie_plots[[1]], width =10 , height =10, dpi = 1000 )
# ggsave(filename = "./Figures/S2/piechart-GOslim-graph-clus_submuc.pdf", plot = pie_plots[[2]], width =10 , height =10, dpi = 1000 )
```

```{r S2b_enrichment_table}
...
# ggsave(paste0("./Figures/S2/","enrichment_table_epi.png"), plot = ggtab[[1]], width = 5, height = 3, dpi = 1000 )
# ggsave(paste0("./Figures/S2/","enrichment_table_submuc.png"), plot = ggtab[[2]], width = 5.5, height = 5, dpi = 1000 )
```
