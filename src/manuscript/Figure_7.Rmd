...
system2(command = "sed", stdout = TRUE,
        args = c("-i", "''","-e", 's/src=\\"\\./src=\\"\\.\\./g',
                 paste0("./md_files/", basename("./Figure_7.md"))))
...
```{r 07c_bulk_UMAP, fig.width=3.74, fig.height=2.5}
#### Tassos r object ####
...
```{r 07b_E2-lvl-gene-heatmap, fig.width=3.5, fig.height=7.48}
library(ComplexHeatmap)
library(circlize)
...
```{r 07d_E2-lvl-top_DEGs, fig.width=3.74, fig.height=2.5}
...
DEGs_E2lvl <- DEGs_E2lvl %>%
  #mutate(Zal)
  left_join(., select(Zal_DEGs, gene="symbol", Zal=`Fold-change`)) %>%
  left_join(., select(sig_PP, gene="symbol", Brad=logFC)) %>%
  left_join(., select(sig_genes, gene, ST=avg_log2FC)) %>%
  mutate(S = case_when(
    is.na(ST) ~ "t",
    isFALSE(is.na(Zal)) & isFALSE(is.na(Brad)) & is.na(ST) ~ "p",
     .default = "no"))
...
```{r Figure-7, fig.width = 7.48, fig.height = 7.48, dpi = 1000}
#############################
