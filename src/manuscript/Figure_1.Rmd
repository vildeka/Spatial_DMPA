...
system2(command = "sed", stdout = TRUE,
        args = c("-i", "''","-e", 's/src=\\"\\./src=\\"\\.\\./g',
                 paste0("./md_files/", basename("./Figure_1.md"))))

# kniter
knit_html_with_job(out_dir = "../lab_book/01_figures", fig_path = "./Figures/FIGURES/")
```
...
```{r 01c_clusters_on_tissue, fig.height=3.3, fig.width=7}
######################
# CLUSTERS ON TISSUE #
######################
...
```{r 01c_st_UMAP_clusters, fig.width=3, fig.height=2.8}
#########################
...
# COMBINE A TO C #
##################
# dev.new(width=7.4, height=3.3, noRStudioGD = TRUE)
AB_C <- plot_grid(NA, B1, B2_C[[2]], B2_C[[1]], C2, nrow = 1, rel_widths = c(.4,.2,.2,.2,.2),  labels = c('A','B',NA,'C'))
# ggsave("./Figures/01/clusters_on_tissue.pdf", clus, width = 2, height = 3.3, dpi = 1000) #, bg = "white"
# ggsave("./Figures/01/sp_annot_on_tissue.pdf", clus, width = 2, height = 3.3, dpi = 1000)
```

```{r 01d_top5_marker_genes_morf, fig.height=1.9, fig.width=6.5}
############################
...
```{r 01e_decon_bar_plot, fig.width=6, fig.height=6.6, dev=c('png','pdf')}
##################
# PLOT FUNCTIONS #
##################
...
```{r remove-Unknown-cell-types}
DATA_r <- DATA_r %>%
  filter(!(is.na(.$cell_annot_1) | .$cell_annot_1 == "Unknown"))
```

```{r 01e_sc_reference_UMAP, fig.height=5, fig.width=5}
...
```{r 01f_cell_prop_tissue, fig.height=2.3, fig.width=3.3}
#################################
