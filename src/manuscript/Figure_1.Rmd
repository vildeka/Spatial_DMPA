...
seuratObj <- readRDS(paste0(result_dir,"seuratObj_annotated_celltypes.RDS"))
...
UMAP_ST <- plot_clusters.fun(DATA, cluster="Clusters", txt_size = 10, dot_size = 0.2,
                        color = clus_col, red = "umapharmony", lable_size = 4) + theme_void() +
...
ggsave("./Figures/01/ST_UMAP.png", UMAP_ST, width = 3, height = 2.8, bg = "white", dpi = 1000)
```

```{r dotplott_top5_DEGs_sm_vs_epi, fig.height=2, fig.width=6}
############################
...
ggsave(paste0("./Figures/01/","Marker_genes_morf", ".pdf"), plot,  width = 6.5, height = 1.9)
ggsave(paste0("./Figures/01/","Marker_genes_morf", ".png"), plot,  width = 6.5, height = 1.9, dpi = 1000)
```
...
decon_column_st <- DATA@assays[["misc"]][["cell_annot"]] #%>%
...
```{r Reference_UMAP_with_cell_lables, fig.height=5, fig.width=5}
cell_type <- c("B cell", "T cell","Epithelial","Endothelial", "Fibroblast", "Myeloid", "Granulocyte", "NK cells","Lymphatics", "ILC", "NA")
...
ggsave("./Figures/01/Figure01E.png", UMAP, width = 4.4, height = 4.4, bg = "white", dpi = 400)
```

### Plot deconvolution cell proportion on tissue
```{r cell_prop_tissue, fig.height=6, fig.width=4}
#################################
...
cell_type <- c("B cell", "T cell", "Keratinocyte","Endothelial","Fibroblast", "Myeloid", "Granulocyte", "NK cells","Lymphatics", "ILC", "NA") # "Smooth muscle"
...
```{r crop-img-with-polygon}
d <- DATA@images[["P097"]]@coordinates %>%
  mutate(keep = tissue + epi_1 + epi_2 + epi_3 + SubMuc_1 + SubMuc_2)
  filter(keep < 0)
  

# num[ 1:nrow, 1:ncol, ndim]
i = array(rep(0, 10),dim = c(5,3,2))

remove_bg.fun <- function(i, df){
  i_ = array(1:(dim(i)[1]*dim(i)[2]),dim = dim(i))
  # i_[1:5,1:5,1]
  
  r <-  round(df$imagerow*scale_fact["P097"]) # c(3,4,5)
  c <-  round(df$imagecol*scale_fact["P097"]) # c(2,2,3)
  
  indx <- map2_dbl(r,c, ~i_[.x, .y, 1])
  indx <- c(indx, rep(indx)+15, rep(indx)+15*2)
  setdiff(indx, 1:(dim(i)[1]*dim(i)[2])*3)
  
  i <- modify_at(i, indx, ~ 0 )
  return(i)
}
ma <- DATA@tools$P097 %>% filter(name == "zoom") %>% mutate(across(c("x", "y"), ~round(.)))
l <- expand.grid(min(ma$x):min(ma$x), min(ma$y):max(ma$y))
l <- expand.grid(1:1979, 1:2000)
s <- sp::point.in.polygon(
         point.x = l$Var1,
         point.y = l$Var2,
         pol.x = DATA@tools$P097$x,
         pol.y = DATA@tools$P097$y )
  
write_file(gsub("(.{1979,}?).", "\\1\n", paste0(s, collapse = "")), "./test.txt")


im[[2]] <- remove_bg.fun(im[[2]], df)

a = array(rep(0, 10),dim = c(5,3,2))
...
```{r crop-img}
mask_background.fun <- function(df, img, sampleid){
# df <- DATA@tools$P097[,c("name","path_idx", "x", "y")] %>% 
...
# df <- df %>% split(~name) 
  ...
  im <- map(im, ~ matrix(
      rgb(.x[,,1],.x[,,2],.x[,,3], .x[4,,]* 1), nrow=dim(.x)[1]))
  raster <- rast(im[[1]])
  # Crop the raster using the polygon
  cropped_raster <- mask(raster, rasterized)
  
  # Plot the cropped raster to verify
  plot(cropped_raster)

  image_array <- as.array(cropped_raster)
  DATA@images[["P031"]]@image <<- image_array
}

walk(sampleid, ~mask_background.fun(DATA@tools[[.x]], DATA@images[[.x]], .x) )
