...
g_table_epi <- G_epi %>% as_tibble() %>%
  ...
  filter(count > 4) %>%
  left_join(., select(GO_comb_epi, Term, ES, "p<0.01"), by="Term") %>%
...
GO_degs <- GO_comb_epi %>% 
  ...
  left_join(., select(G_nodes_e, pathway, degree, community), by=c("id"="pathway")) %>%
  arrange(community) %>%
  ...
  summarise(g = sum(weight), # n is number of shared genes with other terms, l is the sum of weights
            n_genes = sum(weight > 0), .by = "from" )
  
  
GO_degs <- GO_comb_sub %>%
  mutate("p<0.05" = pmap(., ~intersect(..10, dge[[..1]]))) %>% 
  left_join(., select(G_nodes_s, pathway, degree, community), by=c("id"="pathway")) %>%
  arrange(community) %>%
  select(Term, community, `p<0.05`) %>%
  unnest(cols = c("p<0.05")) %>%
  mutate(g = paste0(unique(.$`p<0.05`[cur_group_rows()]), collapse = ","), .by="community") %>%
