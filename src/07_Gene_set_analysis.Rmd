...
#KEGG_path <- "../resources/c2.cp.kegg_medicus.v2023.2.Hs.symbols.gmt"
KEGG_path <- "../resources/c2.cp.kegg_legacy.v2023.2.Hs.symbols.gmt"
GO_path <- "../resources/c5.go.bp.v2023.2.Hs.symbols.gmt"
...
fgseaRes_LE <- fgseaRes_df %>% 
  mutate(fgseaRes = pmap(., ~leading_edge(..3, get(paste0(..2,"_database")), gene_rank_list[[..1]]) )) %>%
...
`Rank at max`The position in the ranked list at which the maximum enrichment score occurred. The more interesting gene sets achieve the maximum enrichment score near the top or bottom of the ranked list; that is, the rank at max is either very small or very large. `Tags` to indicate the percentage of genes contributing to the enrichment score (leadingEdge/size\*100), `List` to indicate where in the list the enrichment score is attained, it reports the percentage of genes before (for positive ES) or after (for negative ES) the peak in the running enrichment score. `Signal` for enrichment signal strength.
...
HIV <- read.xlsx("/Users/vilkal/Downloads/Supplementary Table 3 Vilde.xlsx", sheet = "GO_Up", colNames = T, startRow = 2) %>%
  mutate(leadingEdge =  c(gsub(";", ",", .$Genes))) %>% 
  as_tibble() %>% dplyr::rename(pathway="Term") %>% 
  left_join(., goterms, by=c("GOID"="id")) %>% 
  mutate(full = GO_database[.$t]) %>%
  add_count(pathway) %>% 
  mutate("leadingEdge_comb" = paste0(.$leadingEdge[cur_group_rows()], collapse = "|"), .by="pathway") %>%
  slice_min(n=1, order_by = tibble(P.value), by="pathway")

myCollection <- GOCollection(na.omit(HIV$GOID))
slimdf <- goSlim(myCollection, slim, "BP")
slimdf <- mappedIds(slimdf, myCollection, GOMFOFFSPRING) %>% as_tibble(rownames = "SlimID")

write.xlsx(slimdf, "/Users/vilkal/Downloads/HIV_GOslim_summary.xlsx")

m <- get_similarity_matrix(HIV, geneSets=GO_database, semData=d) 
graph <- build_emap_graph(sim_m=m, fgesa_df=HIV, cex_line=1, min_edge=0)
```

```{r}
sig_epi_GO <- fgseaRes[1:4] %>% bind_rows(., .id = "clus") %>% filter(!(grepl("\\d", .$clus))) %>% filter(padj<=0.05) #%>% .$pathway %>% unique()
# sig_epi_GO <- fgseaRes_GO_[1:4] %>% bind_rows(., .id = "clus") %>% filter((grepl("[^\\d]", .$clus))) %>% filter(padj<=0.05)
sig_sub_GO <- fgseaRes[5:10] %>% bind_rows(., .id = "clus") %>% filter(grepl("\\d", .$clus)) %>% filter(padj<=0.05) #%>% .$pathway %>% unique()
```

```{r Similarity_matrix_enrichplot}
...
GO_comb <- sig_epi_GO %>% 
  ...
  add_count(pathway) %>% 
  left_join(goterms, by=c("pathway"="t")) %>%
  # filter leading edge by parameters set above:
  #mutate(leadingEdge = pmap(., ~intersect(..9, dge[[..1]]))) %>%
...
m <- get_similarity_matrix(GO_comb, geneSets=GO_database, semData=d) 
set.seed(123); graph <- build_emap_graph(sim_m=m, fgesa_df=GO_comb, cex_line=1, min_edge=0.1)
...
set.seed(123);G <- graph$tidy %>% 
  ...
  left_join(., select(GO_comb, size, pathway, "Sig. in"), by="pathway") %>%
...
g_table_epi <- G_epi %>% as_tibble() %>%
  ...
  filter(count > 4) %>%
  left_join(., select(GO_comb, Term, ES), by="Term") %>%
...
set.seed(1);p <- G_epi %>%
  left_join(., select(g, pathway, lab="Term", n), by="pathway") %>%
...
GO_comb <- sig_sub_GO %>% 
  filter(!(grepl(submuc_remove, .$clus))) %>%
  #mutate(., clus = factor(l[as.character(.$clus)], levels = ord1)) %>%
  add_count(pathway) %>% 
  left_join(goterms, by=c("pathway"="t")) %>%
  # filter leading edge by parameters set above:
...
tabel_col.fun<- function(g, pal){
  for(i in 1:nrow(g)){
    ...
    ggtab <- table_cell_bg(ggtab, 
                           row = i+1, column = 1, 
                           fill = pal[as.character(col)], color = pal_epi[as.character(col)] )
    # ggtab <- table_cell_bg(ggtab, i+1, column = 4, linewidth = 1,
    #                        color = pal_epi[as.character(col)])
    print(ggtab)
  }}
...
base_size <- 8
g_table <- list("epi"=g_table_epi, "SubMuc"=G_nodes_s)
pal_list <- list("epi"=pal_epi, "SubMuc"=pal_sub)

ggtab <- imap(g_table, ~ .x %>%
  ggtexttable(.[,-1], rows = NULL, 
            theme = # ttheme("light"))
            ttheme(base_style = "light",
                  base_colour = "#616161",
                  padding = unit(c(2, 2), "mm"),
                  ...
                  rownames.style = rownames_style(size = base_size+1),
                  tbody.style = 
                    tbody_style(color = "#616161", size = base_size, fill = "white", parse = T,
                                hjust = as.vector(matrix(c(.5, 0, .5, .5, .5), ncol = 5, nrow = nrow(g), byrow = TRUE)),
                                x = as.vector(matrix(c(0.5, .1, .5,.5, .5), ncol = 5, nrow = nrow(g), byrow = TRUE)) 
                  ) )) %>%
  tabel_col.fun(., pal_list) )
...
set.seed(1);p <- imap(m, 
                      ~mutate(G_epi, col = factor(c(.x, "gray"))[ifelse(grepl(.y, G_nodes_$`Sig. in`), 1, 2)]) ) %>%
...
m <- set_names(c("#00BFC4","#377EB8", "#00A9FF","#CD9600"), c("8", "4", "3", "0"))

set.seed(1);p <- imap(m, 
                      ~mutate(G_sub, col = factor(c(.x, "gray"))[ifelse(grepl(.y, G_nodes_$`Sig. in`), 1, 2)]) ) %>%
...
df <- imap(slim_df$slim, ~ .x %>% # slim_df$slim[[1]] %>%
          filter(!(Percent == 0)) %>%
          arrange(desc(Percent)) %>%
          mutate(Term = factor(.$Term, levels = .$Term)) %>%
          mutate(ystart = c(0, cumsum(Percent)[1:nrow(.)-1])) %>%
          ...
          {. ->> temp}  %>%
          unnest(go_terms) %>%
            #left_join(., select(G_nodes[["epi"]], community, go_terms="Term"), by=c("go_terms")) %>%
...
# tab <- readRDS(paste0(result_dir,"Enrichment_tables.RDS"))
