...
```{r 02d_Clustering_resolution, eval=FALSE, include=FALSE}
##################################
# EVALUATE CLUSTERING RESOLUTION #
##################################
seuratObj <- FindNeighbors(seuratObj, reduction = "harmony", dims = 1:20, k.param = 20, prune.SNN = 1/15) 
...
plot_clusters.fun <- function(obj, cluster, red = "umap_harmony", color = "Brew_all", lable = TRUE){
  ...
  p <- ggplot(feat, aes(!!(red_1), !!(red_2), 
    ...
    #geom_text(data = lable_df, aes(label = !!cluster), col="black") +
    #ggrepel::geom_label_repel(data = lable, aes(label = !!cluster)) +
    guides(color = guide_legend(override.aes = list(size=2, alpha = 1))) +
...
p <- map(res, ~plot_clusters.fun(seuratObj, cluster=.x, lable = "cell_id"))
plot_grid(ncol = 2, 
         plotlist = p)
