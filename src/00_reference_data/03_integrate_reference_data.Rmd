...
```{r Clustering_resolution, eval=FALSE, include=FALSE}
##################################
# EVALUATE CLUSTERING RESOLUTION #
##################################
...
plot_clusters.fun <- function(obj, cluster, red = "umap_harmony", color = "Brew_all"){
  if(color == "Brew_all"){
    pal <- c(scales::hue_pal()(8),
             RColorBrewer::brewer.pal(9,"Set1"),
             RColorBrewer::brewer.pal(8,"Set2"),
  ...
  p <- ggplot(feat, aes(umap_harmony_1, umap_harmony_2, 
                        color = !!cluster), label= TRUE) + 
    geom_point(alpha = 0.5, size=.5) + ggtitle(as_label(cluster)) +
    labs(color= "Clusters") + 
    geom_text(data = lable, aes(label = !!cluster), col="black") +
    #ggrepel::geom_label_repel(data = lable, aes(label = !!cluster)) +
    scale_colour_manual(values = pal)  +
    my_theme + NoLegend()
  return(p)
...
res <- c("RNA_snn_res.0.25", "RNA_snn_res.1")
p <- map(res, ~plot_clusters.fun(seuratObj, .x,))
plot_grid(ncol = 2, 
         plotlist = p)
