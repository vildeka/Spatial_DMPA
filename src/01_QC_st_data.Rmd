...
# kniter
knit_html_with_job(out_dir = lab_dir, fig_path = "./Figures/01/")
```
...
```{r seurat-function, eval=FALSE, include=FALSE}
feature <- c("percent_mito", "nFeature_RNA", "nCount_RNA")
...
```{r 01b_sp_mt_plot, fig.height=3.5, fig.width=7}
...
```{r 01c_sp_feat_plot, fig.height=3.5, fig.width=7}
...
```{r 01d_sp_count_plot, fig.height=3.5, fig.width=7}
...
Select all spots with less than 15% mitochondrial reads, less than 10% hb-reads and 100 detected genes. 
...
### Summary stats
Dimension of DATA before filtering: `r dim(temp)`<br/>
Dimension of DATA after filtering:  `r dim(DATA)`
...
```{r 01e_QC_plot_filtered, fig.height=15}
...
```{r 01f_filtered_spots, fig.height=8, fig.width=7}
