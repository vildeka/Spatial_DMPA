...
## Filtering
Select all spots with less than 15% mitochondrial reads, less than 10% hb-reads and 100 detected genes.<br/>
...
###########################
feature <-  c("nCount_RNA", "nFeature_RNA","percent_mito","percent_hb", "percent_ribo")
sapply(DATA@meta.data[feature], base::summary) %>% 
  as_tibble(rownames = "stat") %>% 
  knitr::kable(digits = 1)
...
```{r 01e_QC_plot_filtered, fig.height=10, fig.width=6}
# dev.new(width=6, height=10, noRStudioGD = TRUE)
################################
# VIOLIN PLOT BEFORE FILTERING #
################################
feature <-  c("nCount_RNA", "nFeature_RNA","percent_mito","percent_hb", "percent_ribo")
p_ <-  map(feature, ~violin.fun(temp, feature=.x, fill="orig.ident", 
                                col_pal=friendly_cols, txt_size=8, dot_size=.5))
# plot_grid(plotlist=p_, ncol = 1)

################################
# VIOLIN PLOT AFTER FILTERING #
################################
p <-  map(feature, ~violin.fun(DATA, feature=.x, fill="orig.ident", 
                               col_pal=friendly_cols, n=1, txt_size=8, dot_size=.5))
...
### Plot filtered spots
```{r 01f_filtered_spots, fig.height=3.5, fig.width=7}
# dev.new(width=7, height=3.5, noRStudioGD = TRUE)
(plots <- temp %>%
  plot_spatial.fun(., 
      sampleid = sample_id,
