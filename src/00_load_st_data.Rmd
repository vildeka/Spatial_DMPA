...
DATA %>%
  plot_spatial.fun(., 
      ...
      annot_line = .25,
      img_alpha = 1,
      point_size = 0) + theme(legend.position = "none")
```
...
```{r 00b_plot_sp_annot, fig.height=8, fig.width=5}
# dev.new(width=5, height=8, noRStudioGD = TRUE)
DATA %>%
  plot_st_meta.fun(.,  
          assay="RNA",
...
```{r 00c_plot_new_sp_annot, fig.height=8, fig.width=5}
# dev.new(width=10, height=10, noRStudioGD = TRUE)
DATA <-  DATA %>%
  mutate(sp_annot = case_when(colnames(DATA) %in% keep_epi ~ 'epi',
                              colnames(DATA) %in% keep_SubMuc ~ 'SubMuc',
...
```{r 02d_plot_spots_to_remove, fig.height=8, fig.width=5}
# dev.new(width=10, height=10, noRStudioGD = TRUE)
...
```{r 00e_final-sp_annot, fig.height=8, fig.width=5}
