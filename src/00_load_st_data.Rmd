...
# Change the figure path from ./Figures/ to ../Figures/:
...
```{r Paulos_manual_annot, eval=FALSE, include=FALSE}
##########################
# ADD MANUAL ANNOTATION #
#########################
DATA$sp_annot <- "other"
...
```{r plot_sp_annot, fig.height=8, fig.width=7}
DATA %>%
  plot_st_meta.fun(.,  
          assay="RNA",
          feat = "sp_annot",
...
```{r add-missing-sp-annot, fig.height=8, fig.width=7}
# dev.new(width=10, height=10, noRStudioGD = TRUE)
DATA <-  DATA %>%
  mutate(sp_annot = case_when(colnames(DATA) %in% keep_epi ~ 'epi',
                              colnames(DATA) %in% keep_SubMuc ~ 'SubMuc',
...
```{r plot_spots_to_remove, fig.height=8, fig.width=7}
# dev.new(width=10, height=10, noRStudioGD = TRUE)
