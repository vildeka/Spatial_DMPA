...
s_col <- c("#FFD92F","#8DA0CB","#FC8D62","#E78AC3","#A6D854","#66C2A5","#B3B3B3","#eb6062","#984EA3","#E5C494","#377EB8","#4DAF4A","#FF7F00","#FFFF33","#A65628","#F781BF")
e_col <- c("#FFF2AE","#CBD5E8","#FDCDAC","#F4CAE4","#E6F5C9","#B3E2CD","#CCCCCC","#FBB4AE","#DECBE4","#F1E2CC","#B3CDE3","#CCEBC5","#FED9A6","#FFFFCC","#E5D8BD","#FDDAEC")
...
meta_group_lvl <- c("B cell", "endothelial", "keratinocyte", "T cell","fibroblasts","immune",  "granulocytes") #dput(sort(unique(df[[meta_group]])))
...
meta_group <- "comb"
subgroup <- "cell_annot_2"
meta_group_lvl <- c("B cell", "T cell", "keratinocyte", "fibroblast","immune", "endothelial") #, "granulocytes", #dput(sort(unique(df[[meta_group]])))
...
df <- decon_column_st %>%
  ...
  mutate(cell_type = ifelse(.$cell_type == "granulocytes", "immune", .$cell_type)) %>%
  mutate(comb = ifelse( grepl("B cell|T cell", .$cell_annot_1), .$cell_annot_1, .$cell_type))  %>%
...
decon_percent <- df %>%
  ...
  arrange(sp_annot) %>%
  pivot_wider(names_from = c("sp_annot"), values_from = "sum" ) %>%
...
decon_percent <- df %>%
  ...
  arrange(ID) %>%
  pivot_wider(names_from = c("ID"), values_from = "sum") %>% 
  group_by(meta_group) %>%
...
ggsave(paste0("./Figures/04/", "decon_bar_plot.png"), p, height=6, width=6.6, bg = "white")
```
