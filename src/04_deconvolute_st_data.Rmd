...
# Facet wrap CLUSTER #
#####################
# I want to plot the intinsity of the cell population in 
clus_exp <- function(spe, clus_spe, clusters, gene, assay="RNA"){
  #c <- unique( pull(spe, as_label(clusters)))
...
```{r coocurrence_df}
coocurrence.fun <- function(spe, gr){
  coocurrence <- lapply(gr,function(x){
    temp <- DATA_st@assays$SCDC@data[,spe$orig.ident == x]
    ...
    return( temp )})
  names(coocurrence) <- gr
  return(coocurrence)
}
gr <- c("P107", "P108", "P114", "P097","P118", "P105", "P080", "P031")
ctrl <- c("P107", "P108", "P114", "P097")
DMPA <- c("P118", "P105", "P080", "P031")
coocurrence_all <- coocurrence.fun(DATA_st, gr)

coocurrence_ctrl <- coocurrence.fun(DATA_st, ctrl)
coocurrence_ctrl <- (coocurrence_ctrl[[1]]+coocurrence_ctrl[[2]]+coocurrence_ctrl[[3]]+coocurrence_ctrl[[3]])/4

coocurrence_DMPA <- coocurrence.fun(DATA_st, DMPA)
coocurrence_DMPA <- (coocurrence_DMPA[[1]]+coocurrence_DMPA[[2]]+coocurrence_DMPA[[3]]+coocurrence_DMPA[[3]])/4

coocurrence_gr <- c(coocurrence_DMPA, coocurrence_ctrl)
names(coocurrence_gr) <- c("DMPA", "ctrl")
```
...
library(igraph)
cell_codetection.fun <- function(DATA_sub, coocurrence, out){
  pdf(paste0(result_dir,out),width = 7*3.5,height = 2*3.5, useDingbats = F)
...
diff_codetection.fun <- function(DATA_sub, 
                                 coocurrence, 
                                 int,
                                 ncol=5, nrow=1){
  pdf(paste0(result_dir,"cell-cell_codetection_diff.pdf"),width = 7*3.5,height = 2*3.5, useDingbats = F)
  ...
  for(i in names(coocurrence)){
    l <-  t(sapply( as.character(unique(DATA_sub$Clusters)) , 
                            ...
                            cl1=DATA_sub$Clusters, function(jj,reds,cl1) { pmean(reds[cl1==jj,])  }))
    plot_meta( DATA_sub,red = "umap_harmony",feat = "Clusters",cex=.8,label = F,col = "grey90",main=i)
    #int <- (coocurrence[["P031"]]+coocurrence[["P105"]]+coocurrence[["P118"]]+coocurrence[["P080"]])/4
    temp <- log2( ( coocurrence[[i]] +1) /  (int +1) ) # sum 
    if(sum(abs(temp)) != 0){
...
cell_codetection.fun(DATA_r, coocurrence_gr, "cell-cell_codetection_gr.pdf")
cell_codetection.fun(DATA_r, coocurrence_all, "cell-cell_codetection_id.pdf")

# difference in co-ocurrence
diff_codetection.fun(DATA_r, coocurrence=list("Diff"=coocurrence_DMPA), int=coocurrence_ctrl, nrow = 1)
```
...
temp <- DATA_st@assays$SCDC@data
rownames(temp) <- paste0("c",rownames(temp))
#group <- factor(DATA_st$orig.ident[grepl("P107|P108|P114|P097",DATA_st$orig.ident)])
#treat <- factor(DATA_st$orig.ident[grepl("P118|P105|P080|P031",DATA_st$orig.ident)])
...
temp2 <- data.frame(groups=factor(DATA_st$groups))
cl <- lapply(rownames(temp),temp=temp,temp2=temp2, function(x,temp,temp2) {
  set.seed(1)
  df <- cbind(temp2,y=temp[x,])
  m <- summary( glm( y ~ groups ,data = df) )
